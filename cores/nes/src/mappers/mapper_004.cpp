...
void Mapper004::save_state(std::vector<uint8_t>& data) {
    ...
    // A12 timing state for cycle-accurate restoration
    data.push_back(m_last_a12 ? 1 : 0);
    // Store 32-bit values as 4 bytes each (little-endian)
    data.push_back(static_cast<uint8_t>(m_last_a12_cycle & 0xFF));
    data.push_back(static_cast<uint8_t>((m_last_a12_cycle >> 8) & 0xFF));
    data.push_back(static_cast<uint8_t>((m_last_a12_cycle >> 16) & 0xFF));
    data.push_back(static_cast<uint8_t>((m_last_a12_cycle >> 24) & 0xFF));
    data.push_back(static_cast<uint8_t>(m_current_frame_cycle & 0xFF));
    data.push_back(static_cast<uint8_t>((m_current_frame_cycle >> 8) & 0xFF));
    data.push_back(static_cast<uint8_t>((m_current_frame_cycle >> 16) & 0xFF));
    data.push_back(static_cast<uint8_t>((m_current_frame_cycle >> 24) & 0xFF));
    data.push_back(static_cast<uint8_t>(m_irq_pending_at_cycle & 0xFF));
    data.push_back(static_cast<uint8_t>((m_irq_pending_at_cycle >> 8) & 0xFF));
    data.push_back(static_cast<uint8_t>((m_irq_pending_at_cycle >> 16) & 0xFF));
    data.push_back(static_cast<uint8_t>((m_irq_pending_at_cycle >> 24) & 0xFF));
}
...
void Mapper004::load_state(const uint8_t*& data, size_t& remaining) {
    ...
    // Load A12 timing state if present (for newer savestate format)
    if (remaining >= 13) {
        m_last_a12 = (*data++ != 0); remaining--;
        m_last_a12_cycle = static_cast<uint32_t>(data[0]) |
                          (static_cast<uint32_t>(data[1]) << 8) |
                          (static_cast<uint32_t>(data[2]) << 16) |
                          (static_cast<uint32_t>(data[3]) << 24);
        data += 4; remaining -= 4;
        m_current_frame_cycle = static_cast<uint32_t>(data[0]) |
                                (static_cast<uint32_t>(data[1]) << 8) |
                                (static_cast<uint32_t>(data[2]) << 16) |
                                (static_cast<uint32_t>(data[3]) << 24);
        data += 4; remaining -= 4;
        m_irq_pending_at_cycle = static_cast<uint32_t>(data[0]) |
                                  (static_cast<uint32_t>(data[1]) << 8) |
                                  (static_cast<uint32_t>(data[2]) << 16) |
                                  (static_cast<uint32_t>(data[3]) << 24);
        data += 4; remaining -= 4;
    } else {
        // Default values for older savestates
        m_last_a12 = false;
        m_last_a12_cycle = 0;
        m_current_frame_cycle = 0;
        m_irq_pending_at_cycle = 0;
