...
bool Cartridge::load(const uint8_t* data, size_t size) {
    if (size < 0x150) {
        std::cerr << "GB ROM too small" << std::endl;
        return false;
    }
    ...
    // Extract title (at 0x134, up to 16 bytes)
    m_title.clear();
    for (int i = 0; i < 16; i++) {
        char c = data[0x134 + i];
        if (c == 0 || c >= 0x80) break;
    ...
    // Detect system type from CGB flag at 0x143
    uint8_t cgb_flag = data[0x143];
    if (cgb_flag == 0x80 || cgb_flag == 0xC0) {
        m_system_type = SystemType::GameBoyColor;
    } else {
        m_system_type = SystemType::GameBoy;
    }
    ...
    // Allocate RAM
    m_ram.resize(ram_kb * 1024, 0);

    // Create MBC
    m_mbc = MBC::create(m_mbc_type, m_rom, m_ram, m_rom_banks, m_ram_banks);

    m_crc32 = calculate_crc32(data, size);
    m_loaded = true;

    std::cout << (m_system_type == SystemType::GameBoyColor ? "GBC" : "GB")
              << " ROM loaded: " << m_title
              << " (MBC" << m_mbc_type << ", "
              << m_rom_banks * 16 << "KB ROM, "
              << ram_kb << "KB RAM)" << std::endl;

    return true;
}

void Cartridge::detect_mbc(uint8_t cart_type) {
    switch (cart_type) {
        case 0x00:  // ROM ONLY
            m_mbc_type = 0;
            break;
        ...
        case 0x03:  // MBC1+RAM+BATTERY
            m_mbc_type = 1;
            m_has_battery = (cart_type == 0x03);
            break;

        case 0x05:  // MBC2
        case 0x06:  // MBC2+BATTERY
            m_mbc_type = 2;
            m_has_battery = (cart_type == 0x06);
            break;
        ...
        case 0x13:  // MBC3+RAM+BATTERY
            m_mbc_type = 3;
            m_has_battery = (cart_type == 0x0F || cart_type == 0x10 || cart_type == 0x13);
            m_has_rtc = (cart_type == 0x0F || cart_type == 0x10);
            break;
        ...
        case 0x1E:  // MBC5+RUMBLE+RAM+BATTERY
            m_mbc_type = 5;
            m_has_battery = (cart_type == 0x1B || cart_type == 0x1E);
            break;

        default:
            std::cerr << "Unknown cartridge type: " << std::hex << (int)cart_type << std::dec << std::endl;
            m_mbc_type = 0;
            break;
    }
}
...
void Cartridge::write_ram(uint16_t address, uint8_t value) {
    ...
    } else if (address < m_ram.size()) {
        m_ram[address] = value;
    }
}

void Cartridge::write_mbc(uint16_t address, uint8_t value) {
    if (m_mbc) {
...
bool Cartridge::set_save_data(const std::vector<uint8_t>& data) {
    if (!m_loaded || m_ram.empty()) return false;
    size_t copy_size = std::min(data.size(), m_ram.size());
    std::memcpy(m_ram.data(), data.data(), copy_size);
    return true;
}
...
void Cartridge::load_state(const uint8_t*& data, size_t& remaining) {
    ...
    data += m_ram.size();
    remaining -= m_ram.size();
    if (m_mbc) {
        m_mbc->load_state(data, remaining);
    }
}
